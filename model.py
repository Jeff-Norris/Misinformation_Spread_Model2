...
jsonString = csv_to_json('dictionary.csv')
#print(jsonString)
...
class VirusOnNetwork(mesa.Model):
    ...
    def __init__(
        ...
        #self.G.add_nodes_from(range(0, self.num_nodes))

        def add_edges_from_Louisville(G):
            with open('edgelist.csv', 'r') as d:
                reader = csv.reader(d, quotechar=None, escapechar=None, delimiter='|')
                for row in reader:
                    for elements in row:
                        if elements.split(',')[0] != '':
                            G.add_edge(int(elements.split(',')[0]), int(elements.split(',')[-1]))
                        else:
                            pass

        def add_edges_from_IU(G):
            with open('file.json', 'r') as jsonf:
                edge_dict = json.load(jsonf)
                for key in edge_dict:
                    new_key = int(key)
                    for values in edge_dict[key]:
                        #print(key, values)
                        G.add_edge(int(key), int(values))
                        #print(int(key), int(values))
        ...
        #add_edges_from_Louisville(self.G)
        add_edges_from_IU(self.G)
        print(self.G.edges)

        #i=-50
        #for a in self.grid.get_cell_list_contents(self.G.nodes):
            #a.misinformation = jsonString
